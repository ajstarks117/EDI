...
import { 
  X, User, Activity, MapPin, Battery, ShieldAlert, 
  Trash2, Check, Radio, Clock, ShieldCheck, CheckCircle, 
  QrCode, AlertTriangle, Lock 
} from 'lucide-react';
...
// Draw Control Component for Geofences
...
export default function MapPage() {
  ...
  const [drawingCoordinates, setDrawingCoordinates] = useState<[number, number][] | null>(null);
  ...
  const activeAlert = activeTourist?.status === 'critical' 
    ? alertFeed.find(a => a.touristId === activeTourist.id && a.priority === 'P0' && a.status !== 'closed') 
    : null;
  const activeZone = selectedZone && selectedZone !== 'new' ? geofences.find(g => g.id === selectedZone) : null;

  useEffect(() => {
    if (flyToLocation && mapRef.current) {
      mapRef.current.flyTo({ center: [flyToLocation[0], flyToLocation[1]], zoom: 15 });
  ...
  return (
    <div className="relative h-[calc(100vh-8rem)] w-full rounded-xl shadow-2xl border border-surface-border/40 overflow-hidden">
      <Map
        ref={mapRef}
        initialViewState={{
        ...
        onClick={() => {
          if (!isDrawingMode && !drawingCoordinates) {
            setRightPanelOpen(false);
            selectTourist(null);
            setSelectedZone(null);
          }
      ...
      >
        ...
        {Object.values(positions).map(pos => (
          ...
          >
            <svg width="24" height="24" viewBox="0 0 24 24" fill={STATUS_COLORS[pos.status]} stroke="#fff" strokeWidth="2" className="cursor-pointer">
        ...
        {activeTourist && activeTourist.trail.length > 1 && (
          <Source id="tourist-trail" type="geojson" data={{
            type: 'Feature',
            properties: {},
            geometry: {
              type: 'LineString',
              coordinates: activeTourist.trail
            }
          }}>
            <Layer
              id="tourist-trail-line"
              type="line"
              paint={{
                'line-color': '#818CF8',
                'line-width': 3,
        ...
        {geofences.filter(g => activeFilters.includes(g.type)).map(zone => (
          <Source key={`src-${zone.id}`} id={`src-${zone.id}`} type="geojson" data={{
            type: 'Feature',
            ...
            <Layer
              ...
              type="fill"
              paint={{
                'fill-color': ZONE_COLORS[zone.type].fill,
                'fill-opacity': 0.3
              }}
            />
            <Layer
              ...
              type="line"
              paint={{
                'line-color': ZONE_COLORS[zone.type].stroke,
                'line-width': 2
              }}
            />
          </Source>
                  ...
                  <div className="p-5 space-y-6">
                    ...
                    {!activeTourist.isIdentityVerified && (
                      <button 
                        ...
                        className="w-full flex items-center justify-center space-x-2 py-2.5 bg-surface-bg hover:bg-surface-border border border-indigo-500/30 text-indigo-400 rounded-lg text-sm font-semibold transition focus:outline-none focus:ring-2 focus:ring-indigo-500"
                      >
